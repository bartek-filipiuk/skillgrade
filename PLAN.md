...
- **Podział modeli (pole `model` taska w BUILD_STATE):** `claude-opus-4-8` → deleguj implementację przez Agent tool z `model: "opus"`; w prompcie subagenta zamieść: pełną sekcję "Task N" z `docs/superpowers/plans/2026-07-15-trust-skill-v1.md`, sekcję `## Global Constraints` stamtąd, listę plików i komendę gate'a; subagent NIE commituje — commit robi konduktor po zielonym gate. `claude-fable-5` → wykonaj inline (konduktor) lub Agent `model: "fable"`.
